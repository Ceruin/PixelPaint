...
// Returns { canvas, r }: the pivot sits at pixel (r, r) of the canvas.
export function rotated(src, rect, pivot, deg, kick = 0) {
  ...
  const big = scale2x(scale2x(p, w, h), w * 2, h * 2), W4 = w * 4, H4 = h * 4;
  const r = Math.ceil(Math.max(...[[0, 0], [w, 0], [0, h], [w, h]].map(([x, y]) => Math.hypot(x - gx, y - gy)))) + 1, S = r * 2 + 1;
  const c = document.createElement('canvas'); c.width = c.height = S;
  const x = c.getContext('2d'), img = x.createImageData(S, S), out = new Uint32Array(img.data.buffer);
  ...
  for (let j = 0; j < S; j++) for (let i = 0; i < S; i++) {
    ...
    if (X >= 0 && Y >= 0 && X < W4 && Y < H4) out[j * S + i] = big[Y * W4 + X];
  }
  x.putImageData(img, 0, 0);
  if (cache.size > 160) cache.delete(cache.keys().next().value);
  const res = { canvas: c, r };
  cache.set(key, res);
  return res;
}

// A damped pendulum hanging from a moving pivot (px, px/s²): swinging the pointer swings her.
// th > 0 turns her clockwise (her body swings left), as rotated() does.
export class Pendulum {
  th = 0; om = 0;
  step(dt, ax, ay, len) {
    const al = (-(1800 - ay) * Math.sin(this.th) + ax * Math.cos(this.th)) / Math.max(20, len) - 2.2 * this.om;
    this.om += al * dt; this.th = Math.max(-1.1, Math.min(1.1, this.th + this.om * dt));
  }
}
