...
// Pyxl's care card: needs, mood, level, and Feed / Pet / Play / Nap.
export function openCareCard(pyxl) {
  if (card) return;
  const s = pyxl.stats, bars = {}, mood = h('p.pc-mood'), lvl = h('span.pc-lvl'), napBtn = h('button.btn.sm', { type: 'button' });
  ...
  card = h('div.pyxl-card', {},
    h('div.pc-head', {}, h('strong', {}, 'Pyxl'), lvl, h('span.spacer'), h('button.ibtn.sm', { type: 'button', 'aria-label': 'Close', onclick: close }, icon('x'))),
    mood,
    ...
    h('div.pc-actions', {},
      ...
      h('button.btn.sm', { type: 'button', onclick: () => { close(); pyxl.playGame(); } }, iconCanvas('star', 2), 'Play'),
      napBtn),
    snacks);
